...
* @param G is a the graph G:=(V,E)
*/
void print_graph(const unsigned int n, const int * const G, FILE * output)
{
	int v1, v2;
	FOR(v1, 0, n - 1)
	{
...
* @param p matrix of predecessors p(G)
*/
bool check_paths(const unsigned int n, const int * const G, const int * const d, const int * const p, FILE * output)
{
	int i, j;
	FOR (i, 0, n - 1)
	{
...
void calculate_distance(const unsigned int n, const int * const G, FILE * output)
{
	int max_dist = 0, dist_count = 0, dist_sum = 0, dist;
	bool connected = true;
	int v1, v2;
	FOR(v1, 0, n - 1)
	{
	...
	fprintf(output, "Diameter : %d\n", max_dist);
	fprintf(output, "Average distance : %.5f\n", dist_sum / (double) dist_count);
	fprintf(output, "Connected : %d\n", connected);
}
...
int main(int argc, char **argv)
{
	...
	*/
	...
	//FILE *input = fopen(prefix, "r");
	FILE *standard_output = stdout;//fopen(strcat(prefix, ".output_standard.txt"), "wb");
	FILE *calculus_output = stdout;//fopen(strcat(prefix, ".output_calculus.txt"), "wb");
	...
	if (gDebug)
	{
		...
		print_graph(V, G, standard_output);
	}
	...
	{
		fprintf(standard_output, "\nResult short path:\n");
		print_graph(V, d, standard_output);
	}
	
	if (gPrint) 
	{
		fprintf(standard_output, "\nResult predecessors:\n");
		print_graph(V, p, standard_output);
	}

	elapsedTime /= 1000;
	//fprintf (standard_output, "Time : %f s\n", elapsedTime);
	
	if (check)
	{
		if (check_paths(V, G, d, p, standard_output))
		{
			 fprintf(standard_output, "\nResult are correct:\n");
		}
		else
	...
	calculate_distance(V, d, calculus_output);
